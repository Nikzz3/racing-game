...
- Samples are stamped on **arrival**, never with the sample time a client sends
  for other clients to interpolate its car by, which the sender controls. Honest
  updates delivered in one TCP read therefore sit a millisecond apart. The
  window's duration is floored at 250 ms when judging, never skipped: a
  sub-metre honest hop reads as a few m/s, while a checkpoint-sized teleport
  still reads in the hundreds. Skipping young windows would let a client idle
  past the lap-time floor and then burst the remaining checkpoints before the
  start-line reset discards the evidence.
