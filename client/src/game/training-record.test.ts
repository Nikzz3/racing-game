...
describe("training_progress.json — plateau record", () => {
  ...
  it.skipIf(!progressExists)(
    "eval_laps show improving Python-env lap times across training",
    () => {
...
describe("train_config.json — hyperparameter record", () => {
  ...
  it.skipIf(!configExists)(
    "results.policy_validated_lap_time_s beats the autopilot baseline",
    () => {
      ...
      expect(results.policy_validated_lap_time_s).toBeLessThan(
        results.autopilot_baseline_lap_time_s,
      );
    },
  );
  ...
  it.skipIf(!configExists || !existsSync(PROGRESS_PATH))(
    ...
    () => {
      ...
      const { validated_record } = loadProgress();
      expect(
        Math.abs(results.policy_validated_lap_time_s - validated_record.lap_time_s),
      ).toBeLessThan(0.1);
    },
  );
});
