...
function insertInto(queries: RecordedQuery[], table: string): RecordedQuery {
...
describe("submitLap variant persistence", () => {
  ...
  it("snapshots the session's Variant on both best_laps and replays", async () => {
    ...
    const bestLaps = insertInto(queries, "best_laps");
    expect(bestLaps.text).toContain("variant");
    expect(bestLaps.values).toContain("taxi");
    const replays = insertInto(queries, "replays");
  ...
  it("re-snapshots the Variant when an improved lap replaces the row", async () => {
    await submitLap("Ava", "sunset-ridge", "medium", 61_000, FRAMES, "taxi");
    expect(insertInto(queries, "best_laps").text).toContain("variant = EXCLUDED.variant");
    expect(insertInto(queries, "replays").text).toContain("variant = EXCLUDED.variant");
  });
  ...
  it("writes NULL when the session never declared a Variant", async () => {
    ...
    expect(insertInto(queries, "best_laps").values).toContain(null);
    expect(insertInto(queries, "replays").values).toContain(null);
  });
...
describe("getReplay variant round-trip", () => {
  it("returns the stored Variant with the frames", async () => {
    ...
    expect(await getReplay("Ava", "sunset-ridge", "medium")).toEqual({
      timeMs: 61_000,
      frames: FRAMES,
      variant: "taxi",
    });
  ...
  it("returns an absent variant for a NULL column (legacy rows)", async () => {
    ...
    expect((await getReplay("Ava", "sunset-ridge", "medium"))?.variant).toBeUndefined();
  });
